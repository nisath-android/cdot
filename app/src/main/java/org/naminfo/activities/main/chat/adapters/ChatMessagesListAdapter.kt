...
import org.naminfo.activities.main.chat.data.EventData
import org.naminfo.activities.main.chat.data.EventLogData
import org.naminfo.activities.main.chat.data.OnContentClickedListener
import org.naminfo.activities.main.viewmodels.ListTopBarViewModel
import org.naminfo.databinding.ChatEventListCellBinding
import org.naminfo.databinding.ChatMessageListCellBinding
import org.naminfo.databinding.ChatMessageLongPressMenuBindingImpl
import org.naminfo.databinding.ChatUnreadMessagesListHeaderBinding
...
class ChatMessagesListAdapter(
    ...
    val openFileContentEvent: MutableLiveData<Event<String>> by lazy {
        MutableLiveData<Event<String>>()
    }
    ...
    private val contentClickedListener = object : OnContentClickedListener {
        ...
        override fun onFileClicked(msg: String) {
            openFileContentEvent.value = Event(msg)
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            EventLog.Type.ConferenceChatMessage.toInt() -> {
                Log.i(
                    "[Chat Messages Adapter]  onCreateViewHolder() -> ConferenceChatMessage-> createChatMessageViewHolder()"
                )
                createChatMessageViewHolder(parent)
            }

            else -> {
                Log.i("[Chat Messages Adapter] onCreateViewHolder() -> createEventViewHolder()")
    ...
    private fun createChatMessageViewHolder(parent: ViewGroup): ChatMessageViewHolder {
        ...
        Log.i("[Chat Messages Adapter] createChatMessageViewHolder() -> ChatMessageViewHolder()")
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            is ChatMessageViewHolder -> {
                Log.i(
                    "[Chat Messages Adapter]-2 onBindViewHolder->ChatMessageViewHolder [${eventLog.eventLog.chatMessage?.utf8Text}]"
                )
            ...
            is EventViewHolder -> {
                Log.i(
                    "[Chat Messages Adapter]-3 onBindViewHolder->EventViewHolder [${eventLog.eventLog.chatMessage?.utf8Text}]"
                )
                holder.bind(eventLog)
            }
        }
    ...
    fun splitReplyMsg(input: String): List<String> {
        val regex = """(.*?)\s*\[messageId:\s*([^]]+)]\s*(.*)""".toRegex()
        val matchResult = regex.find(input)

        return if (matchResult != null) {
            val (part1, messageId, part2) = matchResult.destructured
            listOf(part1.trim(), messageId.trim(), part2.trim())
        } else {
            listOf(input) // Return input as a single element if pattern not found
        }
    }
        ...
        fun bind(eventLog: EventLogData) {
            with(binding) {
                if (eventLog.eventLog.type == EventLog.Type.ConferenceChatMessage) {
                    ...
                    val targetMessageId = chatMessage.messageId
                    if (!listOfMsgId.contains(targetMessageId)) {
                        listOfMsgId.add(targetMessageId)
                    }
                    val list = splitReplyMsg(chatMessage.utf8Text ?: "")

                    if (list.size == 1) {
                    } else if (list.size == 2) {
                        Log.i(
                            "[######]",
                            " ==>>list.size:${list.size} ,msgID:$list||$targetMessageId\n" +
                                "chattext:${chatMessageData.chatMessage.utf8Text}\n" +
                                "listOfMsgId:$listOfMsgId"
                        )
                    } else if (list.size == 3) {
                        val isMessageIdExists = listOfMsgId.any { it == list[1] }
                        Log.i(
                            "[######]",
                            " ==>>list.size:${list.size} ,msgID:$list||${targetMessageId}\n" +
                                "listOfMsgId:$listOfMsgId \n" +
                                "isReply:${chatMessage.isReply} \n" +
                                "chat-text:${chatMessage.utf8Text}\n"
                        )

                        if (isMessageIdExists) {
                            Log.i("[######]", " Message ID exists in the list")
                        } else {
                            Log.i("[######]", " Message ID not found")
                        }
                        // data = ChatMessageData(chatMessage)
                    }
                    // data = chatMessageData
                    chatMessageData.contactNewlyFoundEvent.observe(viewLifecycleOwner) {
                        it.consume {
                            Log.i(
                                "[######]-1 contactNewlyFoundEvent=> Binding message [${chatMessage.utf8Text}]"
                            )
                            Log.i(
                                "[######] -2 contactNewlyFoundEvent=> Binding message [isReply:${chatMessage.isReply}]\n" +
                            ...
                            binding.root.post {
                                try {
                                    ...
                                    Log.e(
                                        "[######] Can't notify item [$bindingAdapterPosition] has changed: $e"
                                    )
                                }
                            }
                        }
                    ...
                    setReplyClickListener {
                        ...
                        if (reply != null) {
                            Log.i("[Chat Room]", " setReplyClickListener")
                    ...
                    setContextMenuClickListener {
                        val popupView: ChatMessageLongPressMenuBindingImpl =
                        ...
                        popupView.setReplyClickListener {
                            Log.i("[Chat Room]", " reply popup view")
                            replyMessage()
                            popupWindow.dismiss()
                        }
        ...
        private fun reactToMessage(reaction: String) {
            ...
            if (chatMessage != null) {
                ...
                if (ownReaction != null && ownReaction.body == reaction) {
                    Log.i(
                        "[######] Removing our reaction to message [$chatMessage] (previously [$reaction])"
                    )
                    ...
                    Log.i(
                        "[######] Reacting to message [$chatMessage] with [$reaction] emoji"
                    )
    ...
    inner class EventViewHolder(
        ...
        fun bind(eventLog: EventLogData) {
            with(binding) {
                ...
                Log.i(
                    "[######] Binding Event text- eventViewModel = ${eventViewModel.text.value}\n" +
                        "contact-${eventViewModel.contact.value?.name}"
                )
