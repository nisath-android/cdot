...
import org.naminfo.activities.main.chat.data.ChatMessageAttachmentData
import org.naminfo.activities.main.chat.data.ChatMessageAttachmentData1
...
class ChatMessageSendingViewModel(private val chatRoom: ChatRoom) : ViewModel() {
    ...
    var backIsEnable = MutableLiveData<Boolean>(true)
    var isUploading = MutableLiveData<Boolean>(false)
    var isMessageing = MutableLiveData<Boolean>(false)
    var isCancelled = MutableLiveData<Boolean>(false)

    val attachments = MutableLiveData<ArrayList<ChatMessageAttachmentData>>()
    val attachments1 = MutableLiveData<ArrayList<ChatMessageAttachmentData1>>()
    ...
    init {
        ...
        domain = coreContext.core.authInfoList.get(0).domain.toString()
    ...
    fun addAttachment1(path: String, extURL: String) {
        val list = arrayListOf<ChatMessageAttachmentData1>()
        list.addAll(attachments1.value.orEmpty())
        list.add(
            ChatMessageAttachmentData1(path, extURL) {
                removeAttachment1(it)
            }
        )
        attachments1.value = list
    ...
    private fun removeAttachment1(attachment: ChatMessageAttachmentData1) {
        val list = arrayListOf<ChatMessageAttachmentData1>()
        list.addAll(attachments1.value.orEmpty())
        list.remove(attachment)
        attachments1.value = list
    ...
    fun replyContentMessage() {
        ...
        var fileContent = false
        for (attachment in attachments1.value.orEmpty()) {
            val content = Factory.instance().createContent()
            ...
            if (isBasicChatRoom or (corePreferences.preventMoreThanOneFilePerMessage and (fileContent or voiceRecord))) {
                ...
                // message.addFileContent(content)
                fileContent = true
                textToSend.value = attachment.extURL
            }
        }
    ...
    fun startVoiceRecording() {
        ...
        // Reset duration before recording starts
        ...
        // formattedDuration.value = SimpleDateFormat("mm:ss", Locale.getDefault()).format(0) // duration is in ms

        val minutes = (duration / 1000) / 60
        val seconds = (duration / 1000) % 60
        formattedDuration.value = String.format("%02d:%02d", minutes, seconds)
    ...
    private suspend fun uploadImageToServer(
        ...
        return withContext(Dispatchers.IO) {
            try {
                // Create SOAP request
                val request = SoapObject("www.Baytalkitec.com", "UploadFileString_1").apply {
                    addProperty("fileName", fileName)
                    addProperty("filecontent", fileContent)
                    addProperty("bof", bof)
                    addProperty("eof", eof)
                    addProperty("subscriberid", subscriberId)
                ...
                // Transport setup
                val transport = HttpTransportSE("http://192.168.1.31/HTTP/Mobion.asmx", 90000).apply {
                    debug = true
                }

                // Make the SOAP call
                transport.call("www.Baytalkitec.com/UploadFileString_1", envelope)

                // Parse and return the response
                val response = envelope.response as SoapObject
                response
            } catch (e: Exception) {
                Log.e("[Chat Room]", "Error in SOAP call: ${e.message}", e)
                null
            }
        }
    }
    private suspend fun encodeFileToBase64(path: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                val inputStream = FileInputStream(File(path)) // Open file input stream
                val imageBytes = inputStream.readBytes()
                inputStream.close()

                // Convert image bytes to Base64
                Base64.encodeToString(imageBytes, Base64.DEFAULT).also {
                    Log.i("[Chat Room]", "Image encoded to Base64 successfully")
                    Log.i("[Chat Room]", "base64Image - $it")
                }
            } catch (e: Exception) {
                Log.e("[Chat Room]", "Error encoding file to Base64: ${e.message}")
                null
            }
        }
    }
