 ...
 */
package org.naminfo.activities.main
...
import org.naminfo.activities.main.chat.fragments.CallLogsManagement
import org.naminfo.activities.main.sidemenu.fragments.SideMenuFragment
import org.naminfo.activities.main.viewmodels.CallOverlayViewModel
...
import org.naminfo.core.CorePreferences
import org.naminfo.databinding.MainActivityBinding
...
class MainActivity : GenericActivity(), SnackBarActivity, NavController.OnDestinationChangedListener {
    ...
    private var remoteNum: String? = null
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        splashScreen.setKeepOnScreenCondition {
            !isInitialized // keep splash until initialized
        }
        ...
        remoteNum = corePreferences.getRemoteUserPhoneNumber
        localNum = corePreferences.getCurrentUserPhoneNumber
        ...
        coreContext.callErrorMessageResourceId.observe(
            ...
            it.consume { message ->
                ...
                Log.i(
                    "[Main Activity] Call error message: $message || localNum:$localNum || remoteNum:$remoteNum"
                )

                lifecycleScope.launch {
                    if (!localNum.isNullOrEmpty() && !remoteNum.isNullOrEmpty()) {
                        CallLogsManagement.appendCallLogToFTP(
                            server = "192.168.11.3",
                            username = "Administrator",
                            password = "ivrs@123",
                            caller = localNum.orEmpty(),
                            receiver = remoteNum.orEmpty(),
                            callType = "missed",
               ...
               /* if (message.toString() == "Server timeout" || message.toString() == "User is busy" || message.toString() == "Busy") {
                    // chatSendingViewModel.textToSend.value = msg
                    // binding.footer.message.setText(msg)
        ...
        if (coreContext.core.accountList.isEmpty() || corePreferences.firstStart) {
            ...
            intent.hasExtra("Contacts") -> {
                ...
                if (call != null) {
                    ...
                    val callIntent = Intent(
                        this,
                        org.naminfo.activities.voip.CallActivity::class.java
                    )
                    callIntent.addFlags(
    ...
    private fun checkPermissions() {
        ...
        if (!PermissionHelper.get().hasReadPhoneStatePermission()) {
            Log.i("[Main Activity] Asking for READ_PHONE_STATE permission")
            permissionsRequiredList.add(Manifest.permission.READ_PHONE_STATE)
        }

        if (!PermissionHelper.get().hasPostNotificationsPermission()) {
            Log.i("[Main Activity] Asking for POST_NOTIFICATIONS permission")
            permissionsRequiredList.add(Manifest.permission.POST_NOTIFICATIONS)
        }

        if (!PermissionHelper.get().hasWriteContactsPermission()) {
            Log.i("[Main Activity] Asking for WRITE_CONTACTS permission")
            permissionsRequiredList.add(Manifest.permission.WRITE_CONTACTS)
        }
        if (!PermissionHelper.get().hasReadContactsPermission()) {
            Log.i("[Main Activity] Asking for READ_CONTACTS permission")
            ...
            for (i in permissions.indices) {
                when (permissions[i]) {
                    Manifest.permission.RECORD_AUDIO -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i("[Main Activity] RECORD_AUDIO permission has been granted")
                    }
                    Manifest.permission.CAMERA -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i("[Main Activity] CAMERA permission has been granted")
                        coreContext.core.reloadVideoDevices()
                    }
                    Manifest.permission.WRITE_EXTERNAL_STORAGE -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i(
                            "[Main Activity] WRITE_EXTERNAL_STORAGE permission has been granted, taking snapshot"
                        )
                    }
                    Manifest.permission.READ_PHONE_STATE -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i("[Main Activity] READ_PHONE_STATE permission has been granted")
                        coreContext.initPhoneStateListener()
                        checkPermissions() // Continue asking for other permissions
                    }
                    Manifest.permission.WRITE_CONTACTS -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i("[Main Activity] WRITE_CONTACTS permission has been granted")
                        checkPermissions() // Continue asking for other permissions
                    }
                    Manifest.permission.READ_CONTACTS -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        Log.i("[Main Activity] READ_CONTACTS permission has been granted")
                        checkPermissions() // Continue asking for other permissions
                    }
                    Manifest.permission.POST_NOTIFICATIONS -> if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
