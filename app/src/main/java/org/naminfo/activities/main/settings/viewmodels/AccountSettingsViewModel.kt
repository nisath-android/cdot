 ...
 */
package org.naminfo.activities.main.settings.viewmodels
...
import org.naminfo.activities.main.settings.SettingListenerStub
...
class AccountSettingsViewModel(val account: Account) : GenericSettingsViewModel() {
    ...
    val displayNameListener = object : SettingListenerStub() {
        override fun onTextValueChanged(newValue: String) {
            ...
            if (identity != null) {
                ...
                Log.i("[Account Settings]  new value Display name is $newValue")
                Log.i("[Assistant] [Generic Login]  new value Display name is $newValue")
                params.identityAddress = identity
                account.params = params
            } else {
                Log.e("[Account Settings] Account doesn't have an identity yet")
    ...
    // isDefault mutable is above linphone.db
    ...
    private fun deleteAccount(account: Account) {
        ...
        // Loop through each chat room and delete its history
        for (chatRoom in chatRooms) {
            Log.i("[Account Settings] Chat room info $chatRoom, removing it.")
  ...
  /*          val params = account.params.clone()
    ...
    private fun update() {
        ...
        if (identityAddress != null) {
            displayName.value = identityAddress.displayName ?: corePreferences.getCurrentUserName
            Log.i(
                "[Assistant] [Generic Login] 1-displayName is ${displayName.value}||${ corePreferences.getCurrentUserName}"
            )
        ...
        Log.i(
            "[Assistant] [Generic Login] AccountSettingsViewModel->User name is ${userName.value} | ${params.identityAddress?.displayName}|${corePreferences.getCurrentUserName}"
        )
