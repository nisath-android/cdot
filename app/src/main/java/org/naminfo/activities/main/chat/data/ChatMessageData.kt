 ...
 */
package org.naminfo.activities.main.chat.data
...
import org.naminfo.utils.PatternClickableSpan
...
class ChatMessageData(val chatMessage: ChatMessage) : GenericContactData(chatMessage.fromAddress) {
    ...
    init {
        ...
        Log.i(
            "[*****]",
            " init-isReply:${chatMessage.isReply} || ${chatMessage.messageId}"
        )
    ...
    fun updateBubbleBackground(hasPrevious: Boolean, hasNext: Boolean) {
        Log.i(
            "[######]",
            " => updateBubbleBackground() hasPrevious:$hasPrevious hasNext:$hasNext " +
                "chatMessage.isOutgoing:${chatMessage.isOutgoing}"
        )
    ...
    private fun updateContentsList() {
        ...
        for (index in contentsList.indices) {
            ...
            if (content.isFileTransfer || content.isFile || content.isIcalendar) {
                Log.i("[Chat Room Message]", " filename:${content.name}")
                Log.i("[Chat Room Message]", " filePath:${content.filePath}")
                Log.i("[Chat Room Message]", " type:${content.type}")
                Log.i("[Chat Room Message]", " subtype:${content.subtype}")
                Log.i("[Chat Room Message]", " utf8Text:${content.utf8Text}")
                Log.i("[Chat Room Message]", "isReply:${chatMessage.isReply}")
                Log.i("[Chat Room Message]", "isRead:${chatMessage.isRead}")
                Log.i("[Chat Room Message]", "isForward:${chatMessage.isForward}")
                // chatMessage.addUtf8TextContent(content.name ?: "")
                val data = ChatMessageContentData(chatMessage, index)
                data.listener = contentListener
                list.add(data)
                // text.value = Spannable.Factory.getInstance().newSpannable(content.name)
                val spannable = Spannable.Factory.getInstance().newSpannable("${content.name}\n")
                // Remove existing underlines
                val underlineSpans = spannable.getSpans(
                    0,
                    spannable.length,
                ...
                Log.i("[Chat Room Message]", " elseif-isText:- utf8Text:${content.utf8Text}")
                Log.i("[Chat Room Message]", "elseif- type:${content.type}")
                Log.i("[Chat Room Message]", "elseif- subtype:${content.subtype}")
                Log.i("[Chat Room Message]", "elseif- isReply:${chatMessage.isReply}")
                Log.i("[Chat Room Message]", "elseif- isRead:${chatMessage.isRead}")
                Log.i("[Chat Room Message]", "elseif- isForward:${chatMessage.isForward}")
                ...
                val FILE_PATTERN = Pattern.compile(
                    "\\b\\w+_\\d+\\.[a-zA-Z0-9]{3,4}\\b" // Matches "prefix_numbers.extension"
                )
                text.value = PatternClickableSpan()
                    ...
                    .add(
                        ...
                        object : PatternClickableSpan.SpannableClickedListener {
                            override fun onSpanClicked(text: String) {
                                Log.i("[Chat Message Data] Clicked on file: $text")
                                contentListener?.onFileClicked(text) // Add this to your interface
                            }
                        }
