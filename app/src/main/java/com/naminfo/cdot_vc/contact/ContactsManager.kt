...
import com.naminfo.cdot_vc.utils.TimestampUtils
...
private const val TAG = "[Contact-ContactsManager]"
class ContactsManager(private val context: Context) {
    ...
    fun updateLocalContacts() {
        ...
        for (account in coreContext.core.accountList) {
            ...
            Log.i(
                "[Contacts Manager] friend.address:${friend.address?.displayName}|| friend.name: ${friend.name}"
            )
    ...
    private fun initSyncAccount() {
        ...
        if (accounts.isEmpty()) {
            val newAccount = Account(
                context.getString(R.string.sync_account_name),"accuntType"
                /*context.getString(
    ...
    fun getAvailableSyncAccounts(): List<Triple<String, String, Drawable?>> {
        ...
        for (syncAdapter in syncAdapters) {
            if (syncAdapter.authority == "com.android.contacts" && syncAdapter.isUserVisible) {
                if (syncAdapter.supportsUploading() || syncAdapter.accountType ==
                    /*context.getString(R.string.sync_account_type)*/"accountType"
                ) {
                    Log.i(
