...
import com.naminfo.cdot_vc.activities.main.contact.data.NumberOrAddressEditorData
...
class NativeContactEditor(val friend: Friend) {
    companion object {
        ...
        fun createAndroidContact1(displayName: String): Long {
            Log.i(
                "[Native Contact Editor] Creating a local contact with display name: $displayName"
            )

            val changes = arrayListOf<ContentProviderOperation>()

            // Step 1: Insert a new raw contact without an account (local contact)
            changes.add(
                ContentProviderOperation.newInsert(RawContacts.CONTENT_URI)
                    .withValue(RawContacts.ACCOUNT_NAME, null) // No account name
                    .withValue(RawContacts.ACCOUNT_TYPE, null) // No account type
                    .build()
            )

            // Step 2: Insert the display name into the Data table
            changes.add(
                ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
                    .withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0) // Reference the first operation
                    .withValue(
            ...
            return try {
                ...
                Log.i("[Native Contact Editor] Raw contact creation result is $rawContactUri")
                if (rawContactUri != null) {
                    ...
                    cursor?.use {
                        if (it.moveToNext()) {
                            val contactId: Long = it.getLong(0)
                            Log.i("[Native Contact Editor] New contact ID is $contactId")
                            return contactId
                        }
                    }
                }
                0 // Return 0 if no contact ID was found
            } catch (e: Exception) {
                Log.e("[Native Contact Editor] Error creating contact: ${e.message}", e)
                0
            }
        }
    }
    ...
    fun setPresenceInformation(phoneNumber: String, sipAddress: String): NativeContactEditor {
        ...
        if (syncAccountRawId == null) {
            ...
            val insert = ContentProviderOperation.newInsert(RawContacts.CONTENT_URI)
                ...
                .withValue(RawContacts.ACCOUNT_TYPE, /*ppUtils.getString(R.string.sync_account_type)*/"ACCOUNT_TYPE")
    ...
    private fun removeLinphoneOrSipAddress(sipAddress: String) {
        val delete = ContentProviderOperation.newDelete(contactUri)
            .withSelection(
                ...
                arrayOf(
                    friend.refKey,
                    CommonDataKinds.SipAddress.CONTENT_ITEM_TYPE,"linphone"
                    /*AppUtils.getString(R.string.linphone_address_mime_type)*/,
                    sipAddress
                )
            )
    ...
    private fun addPresenceLinphoneSipAddressForPhoneNumber(address: String, detail: String) {
        val insert = ContentProviderOperation.newInsert(contactUri)
            ...
            .withValue(
                ContactsContract.Data.MIMETYPE,"linphone"
                /*AppUtils.getString(R.string.linphone_address_mime_type)*/
            )
            .withValue("data1", address) // value
            .withValue("data2", AppUtils.getString(R.string.app_name)) // summary
